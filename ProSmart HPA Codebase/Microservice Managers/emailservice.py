...
#***************************************************** This file outlines the algorithm for Microservice Manager responsible for the emailservice microservice **********************************************
...
def Analyse (Test_Time, current_replicas, current_cpu, target_cpu, cpu_request, min_replica, Predicted_Demand):
    ...
    if (Test_Time > 10):                                                        # Prediction of Resource Demand starts after 10 seconds after starting load test
        ...
        with open('emailservice_Demand_Model.pkl', 'rb') as f:                          # Loading of trained PROPHET model 
            emailservice_model = pickle.load(f)
        ...
        forecast = emailservice_model.predict(futures)
...
def Execute (microservice_name, Predicted_Demand):
    ...
    with open('ARM_emailservice.txt', 'w') as file:
        for _ in range(4):
            file.write('0\n')
    return
...
def emailservice(Test_Time):
    ...
    with open('ARM_emailservice.txt', 'r') as file:
