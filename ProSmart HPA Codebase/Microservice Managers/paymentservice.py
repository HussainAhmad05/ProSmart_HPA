...
#***************************************************** This file outlines the algorithm for Microservice Manager responsible for the paymentservice microservice **********************************************
...
def Monitor ():
    ...
    while Available_Replicas != Operational_Replicas:               #To ensure available replicas are in operational state
        ...
        try:
            Replicas_CPU_usage = 'kubectl top pods -l app=paymentservice'
...
def Analyse (Test_Time, current_replicas, current_cpu, target_cpu, cpu_request, min_replica, Predicted_Demand):
    ...
    if (Test_Time > 10):                                                        # Prediction of Resource Demand starts after 10 seconds after starting load test
        ...
        with open('paymentservice_Demand_Model.pkl', 'rb') as f:                          # Loading of trained PROPHET model 
            paymentservice_model = pickle.load(f)
        ...
        forecast = paymentservice_model.predict(futures)
...
def Execute (microservice_name, Predicted_Demand):
    ...
    with open('ARM_paymentservice.txt', 'w') as file:
        for _ in range(4):
            file.write('0\n')
    return
...
def paymentservice(Test_Time):
    ...
    with open('ARM_paymentservice.txt', 'r') as file:
